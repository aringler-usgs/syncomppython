...
$saclst="saclst";
$phtimes="./phtimes.csh";
$asc2sac="/home/asluser/SPECFEM3D_V2.0.0/utils/seis_process/ascii2sac.csh";
...
foreach $file (@ARGV) {
  ...
  if ($opt_h and $hdur > $min_hdur) { # convolve source time function
    print SAC "quit\n";
    close(SAC);
    system("/home/asluser/SPECFEM3D_V2.0.0/utils/convolve_stf g $hdur $outfile");
    system("mv ${outfile}.convolved ${outfile}");
    if ($opt_c) {open(SAC,"|sac");}else {open(SAC,"|sac > /dev/null");}
    print SAC "echo on\n r $outfile\n";}
