...
from matplotlib.pyplot import (figure, axes, plot, xlabel,
                               ylabel, title, subplot, legend, savefig, show)
...
def getargs():
    ...
    parser.add_argument('-dataloc', action="store_true", dest="dataloc",
                        default=False, 
                        help="Use /xs0 data location, otherwise use /tr1 also")
        ...
        for sta in stations:
            try:
                ...
                for cursyn in syns:
                    ...
                    curtrace.filter("bandpass", freqmin=userminfre,
                                    freqmax=usermaxfre,
                                    corners=filtercornerpoles)
                    curtrace.integrate()
                    curtrace.integrate()
                ...
                # Set the time series
                tz = numpy.arange(0, vertcomps[0].stats.npts /
                                vertcomps[0].stats.sampling_rate,
                                vertcomps[0].stats.delta)
                # Get a legend and plot the vertical
                synplot = figure(1)
                subplot(311)
                titlelegend = vertcomps[0].stats.network + ' ' +\
                    vertcomps[0].stats.station + ' '
                stime = str(vertcomps[0].stats.starttime.year) + ' ' +\
                    str(vertcomps[0].stats.starttime.julday) + ' ' +\
                    str(vertcomps[0].stats.starttime.hour) + ':' +\
                    str(vertcomps[0].stats.starttime.minute) +\
                    ':' +\
                ...
                titlelegend = titlelegend + ' ' + str(minper) +\
                    '-' + str(maxper) + ' s per.'
                title(titlelegend, fontsize=12)
                vertcomps.sort(['location'])
                for comps in vertcomps.select(component="Z"):
                    curcolor = getcolor(comps.stats.channel,
                                        comps.stats.location)
                    plot(tz, (comps.data*(10**3)), curcolor,
                         label=comps.stats.location + ' ' +
                         comps.stats.channel)
                legend(prop={'size': 6})
                ...
                if debug:
                    ...
                    print(finalstream)
                subplot(312)
                tne = numpy.arange(0, finalstream[0].stats.npts /
                                   finalstream[0].stats.sampling_rate,
                                   finalstream[0].stats.delta)
                for comps in finalstream.select(component="N"):
                    curcolor = getcolor(comps.stats.channel,
                                        comps.stats.location)

                    plot(tne, (comps.data*(10**3)), curcolor,
                         label=comps.stats.location + ' ' +
                         comps.stats.channel)
                legend(prop={'size': 6})
                ...
                savefig(os.getcwd() + '/' + resultdir + '/' + curnet + cursta +
                        str(vertcomps[0].stats.starttime.year) +
                        str(vertcomps[0].stats.starttime.julday) +
                        str(vertcomps[0].stats.starttime.hour) +
                        str(vertcomps[0].stats.starttime.minute) +
