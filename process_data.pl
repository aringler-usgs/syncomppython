...
$saclst="saclst";
$phtimes="/APPS/res/src/SPECFEM3D_V2.0.0/utils/seis_process/phtimes.csh";
...
foreach $file (@ARGV) {
  ...
  if ($opt_i or $opt_R) {  # transfer instrument response
    ...
    if ($opt_i) { # assume pz file in the same dir
      $pzfile=`ls -1 SAC_PZs_${network}_${sta}_${comp}_${khole}* | head -1`;
      chomp($pzfile);
      ...
      @respfiles=glob("$opt_R/RESP.${network}.${sta}.${khole}.${comp}*");
	print "$opt_R/RESP.${network}.${sta}.${khole}.${comp}*";
